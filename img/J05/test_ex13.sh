...
#    Updated: 2019/01/16 16:41:34 by idcornua         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
do
	...
	if [ "$i" == "4" ];then
		TEST="hi everyone i'm jeff kaplan from the overwatch team"
	fi
	if [ "$i" == "5" ];then
		TEST="QGVDYXUREFBFYTXBmWpJELqCQbQKLWYHUKZZJd"
	fi
	if [ "$i" == "6" ];then
		TEST="higuysihopethiswillworkjustfine"
	fi
	if [ "$i" == "7" ];then
		TEST=""
	fi
