...
#    Updated: 2019/01/16 16:40:49 by idcornua         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
do
	...
	if [ "$i" == "4" ];then
		TEST="HI EVERYONE I'M JEFF KAPLAN FROM THE F***ING OVERWATCH TEAM"
	fi
	if [ "$i" == "5" ];then
		TEST="QGVDYXUREFBFYTXBmWpJELqCQbQKLWYHUKZZJd"
	fi
	if [ "$i" == "6" ];then
		TEST="HIGUYIHOPETHATWILLWORKJUSTFINE"
	fi
	if [ "$i" == "7" ];then
		TEST=""
	fi
